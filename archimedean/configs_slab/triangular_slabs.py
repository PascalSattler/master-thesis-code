...
def build_flat_flat(ncells: int) -> pd.DataFrame:
    ...
    df.loc[1, '#translation vectors'] = '[a/2,sqrt(3)*a/2,0]'
    ...
    for i in range(0, ncells):
        ...
        # interactions
        # inter cell
        df.loc[3*i + 0, inter_cols] = f'{i + 1}', f'{i + 1}', '[1, 0, 0]', '[[J,0,0],[0,J,0],[0,0,J]]'


        # edge cases
        if i < ncells-1:
            # inter cell
            df.loc[3*i + 1, inter_cols] = f'{i + 1}', f'{i + 2}', '[0, 0, 0]', '[[J,0,0],[0,J,0],[0,0,J]]'
            df.loc[3*i + 2, inter_cols] = f'{i + 2}', f'{i + 1}', '[1, 0, 0]', '[[J,0,0],[0,J,0],[0,0,J]]'

    write = os.path.join(dir, 'triangular_flat_flat.csv')
